...
class MapOps{
    public static <K extends Comparable<K>, V> Map<K, V> merge(Map<K, V> mapLeft, Map<K, V> mapRight, MergeStrategy<V> mergeStrategy){
        Map<K, V> result = new TreeMap<>(mapLeft);
        mapRight.forEach((e1, e2) -> result.merge(e1, e2, mergeStrategy::merge));
...
public class GenericMapTest {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Mergeable integers
            ...
            MergeStrategy<Integer> mergeStrategy = (l, r) -> Math.max(l, r);
            printMap(MapOps.merge(mapLeft, mapRight, mergeStrategy));
        } else if (testCase == 4) {
            Map<String, String> mapLeft = new HashMap<>();
            Map<String, String> mapRight = new HashMap<>();
